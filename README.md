...
正常委派显式使用后台模式：`run_agent(background=true)` 立即返回 Job ID，Codex 按 `nextPollSeconds` 自动查看紧凑进度，查询间隔自适应为 30、60、120、180 秒；无变化时静默等待，终态后只读取一次 `job_result`。进度只包含阶段、版本、耗时、回合数、最近活动、最近工具名、验证状态和下一次查询时间，不返回原始日志或工具输入。

如果用户明确要求“不轮询”“静默等待”或“完成后再告诉我”，则使用 `run_agent(background=false)`：MCP 请求保持挂起，Claude 完成后只恢复一次 Codex 回合。前台结果默认只返回状态、Job/Session 标识、计划哈希、耗时、回合数、成本、摘要、验证摘要和截断标记；完整事件流写入本地 Job 文件，不进入默认上下文。
...
后台执行返回 job ID，正常流程按 `nextPollSeconds` 使用：

- `job_status`
- `job_result`
- `job_cancel`

后台租约由 MCP 服务内部心跳维护，`job_status` 只读紧凑进度，不承担续约职责。默认查询间隔为 30、60、120、180 秒；阶段或终态变化会反映在 `progressRevision`，无变化时不重复输出。MCP 断开时非持久任务由服务取消；服务异常退出时由 Worker 租约兜底终止。`job_cancel` 可立即终止。只有用户明确要求任务脱离 Codex 继续运行时，才传入 `persistOnDisconnect=true`。
