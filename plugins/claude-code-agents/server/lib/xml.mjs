...
export function buildDelegationPrompt({ agent, task, plan, acceptanceCriteria = '', context = '', browserMode = 'none', browserMcpProfile = '', codexReviewRequired = true }) {
...
<delegation version="1.0">
  ...
  <browser_testing required="${browserMode !== 'none'}" mode="${escapeXml(browserMode)}" mcp_profile="${escapeXml(browserMcpProfile)}">
    <completion_gate>When required, do not report completed unless a real browser was launched, the specified user paths were exercised, and reproducible evidence was recorded.</completion_gate>
  </browser_testing>
  <execution_contract>
