...
正常 `run_agent` 显式使用 `background=true` 并按 `nextPollSeconds` 查看紧凑进度；`job_status` 查询后台状态时也会续租非持久任务。停止轮询后租约会过期并终止 worker；用户明确要求“不轮询”“静默等待”或“完成后再告诉我”时，使用 `background=false` 前台挂起。
