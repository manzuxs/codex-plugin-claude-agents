...
浏览器测试默认不加载额外能力。`run_agent(browserMode=repository)` 使用仓库已有 Playwright/Cypress；`chrome` 显式添加 Claude in Chrome；`mcp` 必须通过 `browserMcpProfile` 选择预配置 profile。浏览器模式沿用正常配置优先级解析出的权限，不强制或覆盖 `permissionMode`；需要避免非交互审批阻塞的用户可自行配置 `QA_ENGINEER_PERMISSION_MODE=bypassPermissions`。不得通过单次工具参数接收任意 MCP 配置路径。
