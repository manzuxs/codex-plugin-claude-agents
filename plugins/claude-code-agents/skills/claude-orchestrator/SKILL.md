...
8. **最终汇报**：分别说明 Codex 的计划、Claude 的实施结果、验证证据和残余风险，并按“阶段结果”模板输出下一阶段执行计划。

## MCP 工具使用规则

- `list_agents`：配置检查或用户询问可用智能体时使用。
- `run_agent`：只有在 `plan` 已经具体且非空时使用。
- `background=false`：正常委派必须使用前台模式；MCP 请求保持挂起，Agent 完成后只恢复一次 Codex 回合。
- `background=true`：只有用户明确要求“后台执行”时才使用；后台租约由 MCP 服务心跳维护，不依赖 Codex 轮询。
- `persistOnDisconnect=true`：只有用户明确要求“Codex 停止后仍继续运行”时才允许；不得自行启用。
- Codex 请求被取消、MCP 连接关闭或后台租约到期时，任务应停止；不要通过 CLI 绕过生命周期控制。
- `job_status`：仅在用户主动要求查看后台任务时查询；不得为了续约自动轮询，也不得在没有状态变化时重复查询。
